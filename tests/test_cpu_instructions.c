...
START_TEST (test_push)
{
    ...
    cs->reg[2] = 0x22;
    int old_sp = cs->sp;
    ck_assert_int_eq(stepCPU(cs), 11);
...
START_TEST (test_xthl)
{
    ...
    cs->reg[5] = 0x33;
    cs->reg[6] = 0x44;
    ck_assert_int_eq(stepCPU(cs), 18);
...
START_TEST (test_sta)
{
    ...
    cs->memory[2] = 0x05;
    cs->reg[0] = 0x12;
    ck_assert_int_eq(stepCPU(cs), 13);
...
START_TEST (test_shld)
{
    ...
    cs->reg[5] = 0xae;
    cs->reg[6] = 0x29;
    ck_assert_int_eq(stepCPU(cs), 16);
...
START_TEST (test_call)
{
    ...
    ck_assert_int_eq(stepCPU(cs), 17);
